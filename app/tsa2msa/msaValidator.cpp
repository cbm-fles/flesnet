...
msaValidator::msaValidator()
    : numTimeslices(0), tsc(0),
      ...
      num_flags_changes(0), num_sys_id_changes(0), num_sys_ver_changes(0),

      hdr_id_seen(), hdr_ver_seen(), eq_id_seen(), flags_seen(), sys_id_seen(),
      sys_ver_seen(),
...
void msaValidator::insert_values_into_seen_sets(
...
void msaValidator::check_against_last_values(
    const fles::HeaderFormatIdentifier& hdr_id,
    const fles::HeaderFormatVersion& hdr_ver,
    const uint16_t& eq_id,
    const fles::MicrosliceFlags& flags,
...
void msaValidator::check_microslice(
    std::shared_ptr<fles::MicrosliceView> ms_ptr, const bool& printInfo) {
