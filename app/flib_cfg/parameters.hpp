...
private:
  void parse_options(int argc, char* argv[]) {
    ...
    for (size_t i = 0; i < _num_flib_links; ++i) {
      ...
      if (vm.count("l" + std::to_string(i) +
                   "_source")) { // set given parameters
        std::string source =
        ...
        if (source == "link") {
          ...
          if (vm.count("l" + std::to_string(i) + "_sys_id") &&
              vm.count("l" + std::to_string(i) + "_sys_ver")) {
