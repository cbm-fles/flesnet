...
public:
    Verificator() = default;
    ~Verificator() = default;
    bool verify_forward(std::vector<std::string> input_archive_paths, std::vector<std::string> output_archive_paths, uint64_t timeslice_size, uint64_t timeslice_cnt, uint64_t overlap = 1);
};