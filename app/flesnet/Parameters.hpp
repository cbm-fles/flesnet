...
public:
	...
	uint32_t typical_content_size() const {
		return typical_content_size_;
	}

	/// Retrieve the shared memory identifier.
	std::string input_shm() const {
		return input_shm_;
	}
	...
	bool generate_ts_patterns() const {
		return generate_ts_patterns_;
	}

	/// Generate timeslices with random sizes
	bool random_ts_sizes() const {
		return random_ts_sizes_;
	}
