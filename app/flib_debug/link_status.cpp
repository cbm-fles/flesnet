...
std::ostream& operator<<(std::ostream& os, flib_link::data_sel_t sel) {
  switch (sel) {
  case flib_link::rx_disable:
    os << "disable";
    break;
  case flib_link::rx_emu:
    os << "    emu";
    break;
  case flib_link::rx_link:
    os << "   link";
    break;
  case flib_link::rx_pgen:
    os << "   pgen";
    break;
  default:
...
int main(int argc, char* argv[]) {
  ...
  std::stringstream ss;
  ss << "\nlink  data_sel  link_active  data_rx_stop  ctrl_rx_stop  "
        "ctrl_tx_stop \n";
  for (size_t i = 0; i < num_links; ++i) {
    ss << "   " << i << "   " << links.at(i)->data_sel() << "            "
