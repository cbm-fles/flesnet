...
int main(int argc, char* argv[]) {
  ...
  while (i < iter) {
    ...
    if (s_msg.words != r_msg.words) {
      cout << "iter " << i << " words missmatch: send " << s_msg.words
           << " recv " << r_msg.words << endl;
    } else {
      for (size_t j = 0; j < r_msg.words; j++) {
        if (s_msg.data[j] != r_msg.data[j]) {
          cout << "iter " << i << " word " << j << " words in msg "
               << r_msg.words << hex << " data missmatch: send 0x"
               << s_msg.data[j] << " recv 0x" << r_msg.data[j] << dec << endl;
        }
      }
    }
