...
private:
    ...
    std::mutex mtx_;
    std::shared_ptr<cbm::Monitor> monitor_{make_shared<cbm::Monitor>()};
    ...
    std::shared_ptr<vector<uint64_t>> nodes_with_buffer_change_{make_shared<vector<uint64_t>>()};
...
public:
    CentralManager(std::string listen_address, std::string monitoring_uri = "") : Node(0, 0), listen_address_(listen_address) {
        if (!monitoring_uri.empty()) {
            monitor_->OpenSink(monitoring_uri);
        }
        eval_worker_ = std::async(launch::async, &CentralManager::eval_thread, this);
