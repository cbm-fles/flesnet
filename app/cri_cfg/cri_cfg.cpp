...
int main(int argc, char* argv[]) {
  try {
    ...
    std::vector<cri::cri_link*> channels = cri->links();
    ...
    L_(debug) << "Tatal CRI channels: " << cri->number_of_links();
    ...
    for (size_t i = 0; i < cri->number_of_links(); ++i) {
      ...
      if (par.channel(i).source == disable) {
        ...
        channels.at(i)->set_data_source(cri::cri_link::rx_pgen);
        ...
        channels.at(i)->set_data_source(cri::cri_link::rx_user);
      }
    }
