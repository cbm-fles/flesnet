...
void fles::NdpbEpochToMsSorter::process()
{
   ...
   while (0 < this->input.size() ) {
      ...
      for (uint32_t uIdx = 0; uIdx < uNbMessages; uIdx ++)
      {
         ...
         {
            std::cout << "+++> Found a GET4 related message in NdpbEpochToMsSorter, ignoring it! "
                      << "Please check your data input to make sure this is OK !!!!" << std::endl;
            continue;
         } // if gDPB related message

         // Ignore the SYNC messages as they are used only to get all EPOCH messages in nDPB
...
/*
                     std::cout << std::hex << (*itMess).getData() << " "
                               << std::hex << static_cast< uint16_t>( content[ content.size() -8 ] ) << " "
                               << std::hex << static_cast< uint16_t>( content[ content.size() -7 ] ) << " "
                               << std::hex << static_cast< uint16_t>( content[ content.size() -6 ] ) << " "
                               << std::hex << static_cast< uint16_t>( content[ content.size() -5 ] ) << " "
                               << std::hex << static_cast< uint16_t>( content[ content.size() -4 ] ) << " "
                               << std::hex << static_cast< uint16_t>( content[ content.size() -3 ] ) << " "
                               << std::hex << static_cast< uint16_t>( content[ content.size() -2 ] ) << " "
