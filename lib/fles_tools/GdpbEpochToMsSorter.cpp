...
//      uint8_t* buff  = msInput->content();
      ...
      for (uint32_t uIdx = 0; uIdx < uNbMessages; uIdx ++)
      {
         ...
         if( true == mess.isHitMsg() || true == mess.isSyncMsg()  ||
             true == mess.isAuxMsg() )
         {
            std::cout << "+++> Found a nDPB related message in GdpbEpochToMsSorter, ignoring it! "
                      << "Please check your data input to make sure this is OK !!!!" << std::endl;
            continue;
         } // if gDPB related message 
         
         if( true == mess.isEpochMsg() )
         {
            // Expand epoch to 64 bits with a cycle counter in higher bits for longer time sorting
            ...
            fulCurrentLongEpoch = (static_cast<uint64_t>(fuCurrentEpochCycle) << 32) + fuCurrentEpoch;
...
*/
         ...
         else if( true == mess.isEpoch2Msg() )
         {
            ...
            {
               std::cout << "++++> Found an epoch2 message with ChipId out of bound in GdpbEpochToMsSorter, ignoring it! "
                         << "Please check your data input to make sure this is OK !!!!" << std::endl
                         << "+++++> Read " << usChipIdx << " VS max. of " <<  kusMaxNbGet4 << std::endl;
               continue;
            } // if( kuMaxNbGet4 <= usChipIdx )
            
            if( 0 == ( (fulMaskGet4 >> usChipIdx) & 0x1 ) )
            {
               std::cout << "++++> Found an epoch2 message from a masked Chip in GdpbEpochToMsSorter, ignoring it! "
                         << "Please check your data input to make sure this is OK !!!!" << std::endl;
               continue;
            } // if( 0 == ( (fulMaskGet4 >> usChipIdx) & 0x1 ) )
            ...
            fulCurrentLongEpoch2[usChipIdx] = (static_cast<uint64_t>(fuCurrentEpochCycle2[usChipIdx]) << 32) 
...
//            fmsMsgBuffer.insert( mess );
                     ...
                     true == mess.isGet4SysMsg() )
            {
               ...
               if( kusMaxNbGet4 <= usChipIdx )
               {
                  std::cout << "++++> Found a message with ChipId out of bound in GdpbEpochToMsSorter, ignoring it! "
                            << "Please check your data input to make sure this is OK !!!!" << std::endl
                            << "+++++> Read " << usChipIdx << " VS max. of " <<  kusMaxNbGet4 << std::endl;
                  continue;
               } // if( kuMaxNbGet4 <= usChipIdx )
               ...
               if( 0 == ( (fulMaskGet4 >> usChipIdx) & 0x1 ) )
               {
                  std::cout << "++++> Found a message from a masked Chip in GdpbEpochToMsSorter, ignoring it! "
                            << "Please check your data input to make sure this is OK !!!!" << std::endl;
                  continue;
               } // if( 0 == ( (fulMaskGet4 >> usChipIdx) & 0x1 ) )
