...
int register_file_bar::get_mem(sys_bus_addr addr, void *dest, size_t dwords) {
...
int register_file_bar::set_mem(sys_bus_addr addr, const void *source,
                               size_t dwords) {
  sys_bus_addr sys_addr = m_base_addr + addr;
  if (((sys_addr + dwords) << 2) < m_bar_size) {
    //memcpy(reinterpret_cast<uint8_t*>(m_bar) + (sys_addr<<2), source,
    //dwords<<2);
