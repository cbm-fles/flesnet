...
#include "InfinibandExceptionUcx.hpp"
#include <array>
#include <chrono>
#include <cstring>
#include <fcntl.h>
...
class IBConnectionGroupUcx : public ConnectionGroupWorker {
public:
  /// The IBConnectionGroup default constructor.
  IBConnectionGroupUcx() {
    ec_ = rdma_create_event_channel();
    if (ec_ == nullptr) {
      throw InfinibandExceptionUcx("rdma_create_event_channel failed");
    }
    fcntl(ec_->fd, F_SETFL, O_NONBLOCK);
  }

  IBConnectionGroupUcx(const IBConnectionGroupUcx&) = delete;
  IBConnectionGroupUcx& operator=(const IBConnectionGroupUcx&) = delete;

  /// The IBConnectionGroup default destructor.
  ~IBConnectionGroupUcx() override {
    for (auto& c : conn_) {
      c = nullptr;
    }
  ...
  void accept(unsigned short port, unsigned int count) {
    ...
    if (err != 0) {
      ...
      throw InfinibandExceptionUcx("id creation failed");
    }
...
#pragma GCC diagnostic pop
    ...
    if (err != 0) {
      ...
      throw InfinibandExceptionUcx("RDMA bind_addr failed");
    }
    ...
    if (err != 0) {
      ...
      throw InfinibandExceptionUcx("RDMA listen failed");
    }
  ...
  void poll_cm_events() {
    ...
    while ((err = rdma_get_cm_event(ec_, &event)) == 0) {
      ...
      if (event_copy.param.conn.private_data != nullptr) {
        ...
        if (private_data_copy == nullptr) {
          throw InfinibandExceptionUcx("malloc failed");
        }
    ...
    if (err != 0) {
      throw InfinibandExceptionUcx("rdma_get_cm_event failed");
    }
  }
  ...
  int poll_completion() {
    ...
    while (ne_total < 1000 &&
      ...
      if (ne < 0) {
        throw InfinibandExceptionUcx("ibv_poll_cq failed");
      }
...
protected:
  ...
  void init_context(struct ibv_context* context) {
    ...
    if (pd_ == nullptr) {
      throw InfinibandExceptionUcx("ibv_alloc_pd failed");
    }

    cq_ = ibv_create_cq(context, num_cqe_, nullptr, nullptr, 0);
    if (cq_ == nullptr) {
      throw InfinibandExceptionUcx("ibv_create_cq failed");
    }

    if (ibv_req_notify_cq(cq_, 0) != 0) {
      throw InfinibandExceptionUcx("ibv_req_notify_cq failed");
    }
  }
...
private:
  ...
  void on_cm_event(struct rdma_cm_event* event) {
    ...
    case RDMA_CM_EVENT_CONNECT_ERROR:
      L_(fatal) << rdma_event_str(event->event);
      throw InfinibandExceptionUcx("could not establish connection");
    case RDMA_CM_EVENT_UNREACHABLE:
      L_(fatal) << rdma_event_str(event->event);
      throw InfinibandExceptionUcx("remote server is not reachable");
