...
#include "TimesliceBuilderUcx.hpp"
...
TimesliceBuilderUcx::TimesliceBuilderUcx(uint64_t compute_index,
...
TimesliceBuilderUcx::~TimesliceBuilderUcx() = default;

void TimesliceBuilderUcx::report_status() {
...
void TimesliceBuilderUcx::request_abort() {
  L_(info) << "[c" << compute_index_ << "] " << "request abort";
...
void TimesliceBuilderUcx::operator()() {
  try {
    // set_cpu(0);
...
void TimesliceBuilderUcx::on_connect_request(struct rdma_cm_event* event) {
  if (pd_ == nullptr) {
    init_context(event->id->verbs);
  }

  assert(event->param.conn.private_data_len >= sizeof(InputNodeInfoUcx));
  InputNodeInfoUcx remote_info = *reinterpret_cast<const InputNodeInfoUcx*>(
      event->param.conn.private_data);

  uint_fast16_t index = remote_info.index;
  assert(index < conn_.size() && conn_.at(index) == nullptr);

  std::unique_ptr<ComputeNodeConnectionUcx> conn(new ComputeNodeConnectionUcx(
...
void TimesliceBuilderUcx::on_completion(const struct ibv_wc& wc) {
  ...
  case ID_RECEIVE_STATUS: {
    ...
    if (connected_ == conn_.size() && in == red_lantern_) {
      auto new_red_lantern = std::min_element(
          ...
          [](const std::unique_ptr<ComputeNodeConnectionUcx>& v1,
             const std::unique_ptr<ComputeNodeConnectionUcx>& v2) {
...
void TimesliceBuilderUcx::poll_ts_completion() {
