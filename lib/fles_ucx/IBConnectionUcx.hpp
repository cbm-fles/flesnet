...
class IBConnectionUcx {
public:
  /// The IBConnection constructor. Creates a connection manager ID.
  IBConnectionUcx(struct rdma_event_channel* ec,
                  uint_fast16_t connection_index,
                  uint_fast16_t remote_connection_index,
                  struct rdma_cm_id* id = nullptr);

  IBConnectionUcx(const IBConnectionUcx&) = delete;
  IBConnectionUcx& operator=(const IBConnectionUcx&) = delete;

  /// The IBConnection destructor.
  virtual ~IBConnectionUcx();
