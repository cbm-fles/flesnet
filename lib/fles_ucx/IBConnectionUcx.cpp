...
IBConnectionUcx::IBConnectionUcx(struct rdma_event_channel* ec,
  ...
  if (cm_id_ == nullptr) {
    ...
    if (err != 0) {
      throw InfinibandExceptionUcx("rdma_create_id failed");
    }
  } else {
    cm_id_->context = this;
  }
...
IBConnectionUcx::~IBConnectionUcx() {
  if (cm_id_ != nullptr) {
...
void IBConnectionUcx::connect(const std::string& hostname,
  ...
  if (err != 0) {
    throw InfinibandExceptionUcx("getaddrinfo failed");
  }
  ...
  if (err != 0) {
    ...
    throw InfinibandExceptionUcx("rdma_resolve_addr failed");
  }

  freeaddrinfo(res);
}

void IBConnectionUcx::disconnect() {
  L_(debug) << "[" << index_ << "] " << "disconnect";
  int err = rdma_disconnect(cm_id_);
  if (err != 0) {
    throw InfinibandExceptionUcx("rdma_disconnect failed");
  }
}

void IBConnectionUcx::on_rejected(struct rdma_cm_event* /* event */) {
  L_(debug) << "[" << index_ << "] " << "connection rejected";

  rdma_destroy_qp(cm_id_);
}

void IBConnectionUcx::on_established(struct rdma_cm_event* /* event */) {
  L_(debug) << "[" << index_ << "] " << "connection established";
}

void IBConnectionUcx::on_disconnected(struct rdma_cm_event* /* event */) {
  L_(debug) << "[" << index_ << "] " << "connection disconnected";
}

void IBConnectionUcx::on_timewait_exit(struct rdma_cm_event* /* event */) {
  L_(debug) << "[" << index_ << "] " << "connection reached timewait_exit";

  rdma_destroy_qp(cm_id_);
}

void IBConnectionUcx::on_addr_resolved(struct ibv_pd* pd, struct ibv_cq* cq) {
  ...
  if (err != 0) {
    throw InfinibandExceptionUcx("creation of QP failed");
  }

  err = rdma_resolve_route(cm_id_, RESOLVE_TIMEOUT_MS);
  if (err != 0) {
    throw InfinibandExceptionUcx("rdma_resolve_route failed");
  }

  setup(pd);
}

void IBConnectionUcx::create_qp(struct ibv_pd* pd, struct ibv_cq* cq) {
  struct ibv_qp_init_attr qp_attr {};
  qp_attr.cap = qp_cap_;
  qp_attr.send_cq = cq;
  qp_attr.recv_cq = cq;
...
void IBConnectionUcx::accept_connect_request() {
  ...
  if (err != 0) {
    throw InfinibandExceptionUcx("RDMA accept failed");
  }
}

void IBConnectionUcx::on_connect_request(struct rdma_cm_event* /* event */,
                                         struct ibv_pd* pd,
                                         struct ibv_cq* cq) {
  create_qp(pd, cq);
  setup(pd);
...
std::unique_ptr<std::vector<uint8_t>> IBConnectionUcx::get_private_data() {
...
void IBConnectionUcx::on_route_resolved() {
  ...
  if (err != 0) {
    ...
    throw InfinibandExceptionUcx("rdma_connect failed");
  }
}

void IBConnectionUcx::dump_send_wr(struct ibv_send_wr* wr) {
  for (int i = 0; wr != nullptr; ++i, wr = wr->next) {
    L_(fatal) << "wr[" << i << "]: wr_id=" << wr->wr_id << " ("
              << static_cast<RequestIdentifierUcx>(wr->wr_id) << ")";
    ...
    for (int j = 0; j < wr->num_sge; ++j) {
      L_(fatal) << "  sg_list[" << j << "] " << "addr=" << wr->sg_list[j].addr;
      L_(fatal) << "  sg_list[" << j << "] "
                << "length=" << wr->sg_list[j].length;
      L_(fatal) << "  sg_list[" << j << "] " << "lkey=" << wr->sg_list[j].lkey;
    }
  }
}

void IBConnectionUcx::post_send(struct ibv_send_wr* wr) {
  ...
  if (err != 0) {
    ...
    throw InfinibandExceptionUcx("ibv_post_send failed");
  }
...
void IBConnectionUcx::post_recv(struct ibv_recv_wr* wr) {
  struct ibv_recv_wr* bad_recv_wr;

  int err = ibv_post_recv(qp(), wr, &bad_recv_wr);
  if (err != 0) {
    L_(fatal) << "ibv_post_recv failed: " << strerror(err);
    throw InfinibandExceptionUcx("ibv_post_recv failed");
  }
