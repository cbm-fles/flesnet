...
#include "InputChannelSenderUcx.hpp"
...
InputChannelSenderUcx::InputChannelSenderUcx(
...
void InputChannelSenderUcx::report_status() {
...
void InputChannelSenderUcx::sync_buffer_positions() {
  for (auto& c : conn_) {
...
void InputChannelSenderUcx::sync_data_source(bool schedule) {
...
void InputChannelSenderUcx::operator()() {
  try {
...
bool InputChannelSenderUcx::try_send_timeslice(uint64_t timeslice) {
...
std::unique_ptr<InputChannelConnectionUcx>
InputChannelSenderUcx::create_input_node_connection(uint_fast16_t index) {
  ...
  std::unique_ptr<InputChannelConnectionUcx> connection(
      new InputChannelConnectionUcx(ec_, index, input_index_, max_send_wr,
                                    max_pending_write_requests));
  return connection;
}

void InputChannelSenderUcx::connect() {
...
int InputChannelSenderUcx::target_cn_index(uint64_t timeslice) {
  return timeslice % conn_.size();
}

void InputChannelSenderUcx::dump_mr(struct ibv_mr* mr) const {
  L_(debug) << "[i" << input_index_ << "] " << "ibv_mr dump:";
...
void InputChannelSenderUcx::on_addr_resolved(struct rdma_cm_id* id) {
  IBConnectionGroupUcx<InputChannelConnectionUcx>::on_addr_resolved(id);
...
void InputChannelSenderUcx::on_rejected(struct rdma_cm_event* event) {
  auto* conn = static_cast<InputChannelConnectionUcx*>(event->id->context);
...
std::string InputChannelSenderUcx::get_state_string() {
  std::ostringstream s;
...
void InputChannelSenderUcx::post_send_data(uint64_t timeslice,
                                           int cn,
...
void InputChannelSenderUcx::on_completion(const struct ibv_wc& wc) {
...
#if WITH_TRACE
    L_(trace) << "[i" << input_index_ << "] " << "write timeslice " << ts
  ...
  default:
    ...
    throw InfinibandExceptionUcx("wc for unknown wr_id");
  }
}
