...
#include "InputChannelConnectionUcx.hpp"
...
InputChannelConnectionUcx::InputChannelConnectionUcx(
...
bool InputChannelConnectionUcx::check_for_buffer_space(
    uint64_t data_size, uint64_t desc_size) const {
  auto data_avail = cn_ack_.data - cn_wp_.data +
                    (UINT64_C(1) << remote_info_.data_buffer_size_exp);
  auto desc_avail = cn_ack_.desc - cn_wp_.desc +
  ...
  L_(trace) << "[" << index_ << "] "
            ...
            << ", avail=" << desc_avail;
#endif
  return (data_size <= data_avail && desc_size <= desc_avail);
}

void InputChannelConnectionUcx::send_data(struct ibv_sge* sge,
                                          int num_sge,
                                          uint64_t timeslice,
...
#if WITH_TRACE
  L_(trace) << "[i" << remote_index_ << "] " << "[" << index_ << "] "
...
bool InputChannelConnectionUcx::write_request_available() const {
  return (pending_write_requests_ < max_pending_write_requests_);
}

void InputChannelConnectionUcx::inc_write_pointers(uint64_t data_size,
                                                   uint64_t desc_size) {
  cn_wp_.data += data_size;
  cn_wp_.desc += desc_size;
}

bool InputChannelConnectionUcx::try_sync_buffer_positions() {
  if (our_turn_) {
    our_turn_ = false;
...
uint64_t InputChannelConnectionUcx::skip_required(uint64_t data_size) const {
...
void InputChannelConnectionUcx::finalize(bool abort) {
  finalize_ = true;
  abort_ = abort;
  if (our_turn_) {
    our_turn_ = false;
...
void InputChannelConnectionUcx::on_complete_write() {
  pending_write_requests_--;
}

void InputChannelConnectionUcx::on_complete_recv() {
...
void InputChannelConnectionUcx::setup(struct ibv_pd* pd) {
  // register memory regions
  mr_recv_ =
      ibv_reg_mr(pd, &recv_status_message_, sizeof(ComputeNodeStatusMessageUcx),
  ...
  mr_send_ = ibv_reg_mr(pd, &send_status_message_,
                        sizeof(InputChannelStatusMessageUcx), 0);
  if (mr_send_ == nullptr) {
    throw InfinibandExceptionUcx("registration of memory region failed");
  }
  ...
  recv_sge.length = sizeof(ComputeNodeStatusMessageUcx);
  ...
  send_sge.length = sizeof(InputChannelStatusMessageUcx);
...
*/
void InputChannelConnectionUcx::on_established(struct rdma_cm_event* event) {
  assert(event->param.conn.private_data_len >= sizeof(ComputeNodeInfoUcx));
  memcpy(&remote_info_, event->param.conn.private_data,
         sizeof(ComputeNodeInfoUcx));

  IBConnectionUcx::on_established(event);
}

void InputChannelConnectionUcx::dereg_mr() {
  if (mr_recv_ != nullptr) {
...
void InputChannelConnectionUcx::on_rejected(struct rdma_cm_event* event) {
  dereg_mr();
  IBConnectionUcx::on_rejected(event);
}

void InputChannelConnectionUcx::on_disconnected(struct rdma_cm_event* event) {
  dereg_mr();
  IBConnectionUcx::on_disconnected(event);
}

std::unique_ptr<std::vector<uint8_t>>
InputChannelConnectionUcx::get_private_data() {
  std::unique_ptr<std::vector<uint8_t>> private_data(
      new std::vector<uint8_t>(sizeof(InputNodeInfoUcx)));

  auto* in_info = reinterpret_cast<InputNodeInfoUcx*>(private_data->data());
  in_info->index = remote_index_;

  return private_data;
}

void InputChannelConnectionUcx::post_recv_status_message() {
#if WITH_TRACE
  L_(trace) << "[i" << remote_index_ << "] " << "[" << index_ << "] "
...
void InputChannelConnectionUcx::post_send_status_message() {
#if WITH_TRACE
  L_(trace) << "[i" << remote_index_ << "] " << "[" << index_ << "] "
