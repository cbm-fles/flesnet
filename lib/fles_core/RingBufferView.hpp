...
public:
    ...
    const T& at(std::size_t n) const { return buf_[n & size_mask_]; }

    /// Retrieve pointer to memory buffer.
    T* ptr() { return buf_; }

    /// Retrieve const pointer to memory buffer.
    const T* ptr() const { return buf_; }
    ...
    std::size_t size_exponent() const { return size_exponent_; }

    /// Retrieve buffer size bit mask.
    std::size_t size_mask() const { return size_mask_; }
