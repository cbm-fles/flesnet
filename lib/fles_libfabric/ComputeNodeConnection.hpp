...
public:
	...
	bool abort_flag() {
		return recv_status_message_.abort;
	}
	...
	struct BufferStatus {
		...
		std::string caption() const {
			return std::string("used/freeing/free");
		}
	...
	BufferStatus buffer_status_data() const {
		return BufferStatus { std::chrono::system_clock::now(), (UINT64_C(1)
				<< data_buffer_size_exp_), send_status_message_.ack.data,
				cn_ack_.data, cn_wp_.data };
	}

	BufferStatus buffer_status_desc() const {
		return BufferStatus { std::chrono::system_clock::now(), (UINT64_C(1)
				<< desc_buffer_size_exp_), send_status_message_.ack.desc,
