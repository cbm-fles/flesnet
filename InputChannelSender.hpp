...
public:
    ...
    {
        size_t min_ack_buffer_size = _data_source.desc_buffer().size()
                                     / _timeslice_size + 1;
...
#pragma GCC diagnostic pop
    ...
    void sender_loop()
    {
        ...
        for (uint64_t timeslice = 0; timeslice < _max_timeslice_number;
            ...
            uint64_t total_length = data_length
                                    + mc_length * sizeof(MicrosliceDescriptor);
    ...
    create_input_node_connection(uint_fast16_t index)
    {
        ...
        unsigned int max_pending_write_requests = std::min(
            ...
            static_cast
            <unsigned int>((_num_cqe - 1) / _compute_hostnames.size()));
...
private:
    ...
    std::string get_state_string()
    {
        ...
        for (unsigned int i = 0; i < _data_source.data_buffer().size(); ++i)
            s << " (" << i << ")" << std::hex
              << _data_source.data_buffer().at(i) << std::dec;
        s << std::endl;
                        ...
                        uint64_t data_length, uint64_t skip)
    {
        ...
        _conn[cn]
            ->send_data(sge, num_sge, timeslice, mc_length, data_length, skip);
    }
    ...
    virtual void on_completion(const struct ibv_wc& wc)
    {
        ...
        case ID_WRITE_DESC: {
            ...
            if (ts == acked_ts)
                ...
                _ack.at(ts) = ts;
            _acked_data = _data_source.desc_buffer()
                              .at(acked_ts * _timeslice_size)
                              .offset;
