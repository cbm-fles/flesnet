...
public:
          ...
          _max_pending_write_requests(max_pending_write_requests)
    {
        ...
        _qp_cap.max_send_sge
            = 4; // max. two chunks each for descriptors and data
    ...
    void wait_for_buffer_space(uint64_t data_size, uint64_t desc_size)
    {
        ...
        while (_cn_ack.data - _cn_wp.data
               + (1 << _remote_info.data_buffer_size_exp) < data_size
                   ...
                   uint64_t mc_length, uint64_t data_length, uint64_t skip)
    {
        ...
        send_wr_tscdesc.wr_id = ID_WRITE_DESC | (timeslice << 24)
                                | (_index << 8);
