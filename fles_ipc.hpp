...
struct TimesliceWorkItem
{
    ...
    uint32_t timeslice_size;       ///< Number of core microslices
    uint32_t overlap_size;         ///< Number of overlapping microslices
...
public:
    ...
    uint64_t core_size() const
    { return _work_item.timeslice_size; }

    /// Retrieve the number of overlapping microslices.
    uint64_t overlap_size() const
    { return _work_item.overlap_size; }

    /// Retrieve the total number of microslices.
    uint64_t total_size() const
    { return _work_item.timeslice_size + _work_item.overlap_size; }
    ...
    {
        return &data(component, desc(component).offset) + total_size() * sizeof(MicrosliceDescriptor)
